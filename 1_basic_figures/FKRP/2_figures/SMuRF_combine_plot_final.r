# Rscript --vanilla SMuRF_combine_plot_final.r normalized_FKRP_block1_func normalized_FKRP_block2_func  normalized_FKRP_block3_func normalized_FKRP_block4_func normalized_FKRP_block5_func normalized_FKRP_block6_func
...
p = p + geom_boxplot() + geom_signif(comparisons = list(c("Synonymous", "Missense")),y_position=4, test=wilcox.test, test.args=c("two.sided"))+geom_signif(comparisons = list(c("Synonymous", "Nonsense")),y_position=4.4,test=wilcox.test, test.args=c("two.sided"))+geom_signif(comparisons = list(c("Synonymous", "Start-loss")),y_position=4.8, test=wilcox.test, test.args=c("two.sided"))+geom_signif(comparisons = list(c("Nonsense", "Start-loss")),y_position=4, test=wilcox.test, test.args=c("two.sided"))
...
variant_count_number=length(readLines(args[1]))+length(readLines(args[2]))+length(readLines(args[3]))+length(readLines(args[4]))+length(readLines(args[5]))+length(readLines(args[6]))-6
p = p + labs(title="FKRP - All Blocks", subtitle=paste(variant_count_number,"variants"))
...
ggsave(filename="SMuRF_FKRP_scores_combined_withnumber.png",dpi=300)
