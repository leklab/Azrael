#Rscript --vanilla SMuRF_gnomadv4_FKRP_revise.r FKRP_DiMSum_annotated_NEW.tsv
...
min_allele_freq <- min(df$gnomad_v4_genome_af)-0.06
max_allele_freq <- max(df$gnomad_v4_genome_af)+0.06
...
ggsave(filename=paste(basename(args[1]),".gnomadv4_geno_jitter_high_confidence.pdf",sep = ""))
