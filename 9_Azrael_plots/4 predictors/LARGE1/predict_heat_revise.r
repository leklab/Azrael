#Rscript --vanilla predict_heat_revise.r FKRP_DiMSum_annotated_NEW_FINAL.tsv
...
heatmap_plot = heatmap_plot + theme(axis.text.y=element_text(size=15,colour="black")) + theme(plot.title = element_text(size = 15, hjust = 0, vjust = 1, face = "bold"),plot.margin = margin(5, 5, 5, 5))
heatmap_plot = heatmap_plot + theme(axis.text.x=element_text(size=15,colour="black"))
...
ggsave(filename=paste(basename(args[1]),".heatmap.pdf",sep = ""),height=12)
