#Rscript --vanilla SMuRF_predictors_revise.r LARGE1_DiMSum_annotated_v6.tsv
...
s = s + theme(axis.text.y=element_text(size=14,colour="black"),axis.title.y=element_text(size=22,colour="black",vjust=1.5))
s = s + theme(axis.text.x=element_text(size=14,colour="black"),axis.title.x= element_text(size=22,colour="black",vjust=1.5))
...
s = s + labs(title="LARGE1", subtitle=paste(variant_count_number,"variants"))


s = s + theme(axis.ticks=element_blank(),plot.title=element_text(size=20),plot.subtitle=element_text(size=18))
...
s = s + theme(legend.text=element_text(colour="black",size=9.5))+ theme(legend.position="top")
s = s + theme(legend.title=element_text(colour="black",size=9.5))
s = s + guides(fill=guide_legend(title="Count"))
s = s + theme(legend.background=element_rect(linetype="solid",colour="black"))


#1#
ggsave(filename=paste(basename(args[1]),".EVE_high_confi.pdf",sep = ""), height=12)

#2#
#2#ggsave(filename=paste(basename(args[1]),".REVEL_high_confi.pdf",sep = ""),height=12)

#3#
#3#ggsave(filename=paste(basename(args[1]),".primateAI_3D_high_confi.pdf",sep = ""), height=12)

#4#
#4#ggsave(filename=paste(basename(args[1]),".MAVERICK.png",sep = ""),dpi=300, height=10)

#5#
#5#ggsave(filename=paste(basename(args[1]),".AlphaMissense_high_confi.pdf",sep = ""), height=10)
