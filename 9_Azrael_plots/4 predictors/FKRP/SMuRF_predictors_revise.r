#Rscript --vanilla SMuRF_predictors_revise.r FKRP_DiMSum_annotated_NEW_FINAL.tsv
...
s = s + theme(axis.text.y=element_text(size=14,colour="black"),axis.title.y=element_text(size=22,colour="black"))
s = s + theme(axis.text.x=element_text(size=14,colour="black"),axis.title.x=element_text(size=22,colour="black"))
...
s = s + theme(legend.text=element_text(colour="black",size=12.2))+ theme(legend.position="top")
s = s + theme(legend.title=element_text(colour="black",size=12.2))
...
#1#
ggsave(filename=paste(basename(args[1]),".EVE_high_confi.png",sep = ""),dpi=300, height=12)
...
#5#
#5#ggsave(file=paste(basename(args[1]),".AlphaMissense_high_confi.pdf",sep = ""),height=10)
