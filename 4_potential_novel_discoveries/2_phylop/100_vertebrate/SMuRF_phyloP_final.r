#Rscript --vanilla SMuRF_phyloP_final.r gnomad_fkrp-1_annotated_phyloP.tsv

#Rscript --vanilla SMuRF_phyloP_final.r gnomad_fkrp-2_annotated_phyloP.tsv

#Rscript --vanilla SMuRF_phyloP_final.r gnomad_large1_annotated_phyloP.tsv
...
#1#
s = ggplot(df, aes(x=phyloP100way_vertebrate, y=log2(functional_score))) + theme_bw()
...
#1#
s = s + ylab("log2(Functional score)")+xlab("phyloP (100way vertebrate)")
...
#1
ggsave(filename=paste(basename(args[1]),".png",sep = ""),dpi=300)


#1#
cor.test(df$functional_score, df$phyloP100way_vertebrate,method="spearman")
