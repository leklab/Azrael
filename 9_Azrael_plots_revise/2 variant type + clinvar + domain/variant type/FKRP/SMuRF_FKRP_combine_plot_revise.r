#Rscript --vanilla SMuRF_FKRP_combine_plot_revise.r FKRP_DiMSum_annotated.tsv
...
p=p+geom_boxplot(width=0.1,outlier.size=3,fill="NA")
...
p = p + theme(axis.text.x=element_text(size=17,colour="black",family="Atlas Grotesk Web Bold"),axis.title.x = element_blank())
...
p= p + theme(plot.title=element_text(family="Atlas Grotesk Web Bold", size=20))
p= p + theme(plot.subtitle=element_text(family="Atlas Grotesk Web Light", size=18))
...
ggsave(filename="SMuRF_FKRP_scores_combined_style_withnumber_revised_high_confidence.png",dpi=300)
