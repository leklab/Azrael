#Rscript --vanilla SMuRF_gnomadv4_FKRP_revise.r FKRP_DiMSum_annotated.tsv
...
df <- df[df$gnomad_v4_genome_af != 0,]
df <- df[df$confidence == "HIGH",]

min_allele_freq <- min(df$gnomad_v4_genome_af)
max_allele_freq <- max(df$gnomad_v4_genome_af)

color_palette <- c("lightblue", "red","darkgreen")

# arrange

df <- (df %>%
         arrange(factor(classification, levels = c("Missense", "Synonymous", "Nonsense"))))
...
# Main plot
scatterplot <- ggplot(df, aes(x = gnomad_v4_genome_af, y = fitness_normalized, color = classification)) +
  ...
  labs(x = "gnomAD v4 genome Allele Frequency",
       y = "SMuRF score",
...
scatterplot = scatterplot + theme(axis.text.y=element_text(size=20,colour="black",family="Atlas Grotesk Web Bold"),axis.title.y=element_text(size=22,colour="black",family="Atlas Grotesk Web Bold",vjust=1.5))
scatterplot = scatterplot + theme(axis.text.x=element_text(size=20,colour="black",family="Atlas Grotesk Web Bold"),axis.title.x = element_text(size=22,colour="black",family="Atlas Grotesk Web Bold",vjust=1.5))
...
ggsave(filename=paste(basename(args[1]),".gnomadv4_geno_jitter_high_confidence.png",sep = ""),dpi=300)
