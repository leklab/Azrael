#python clinvar_patient_reports_LARGE1_revise.py LARGE1_DiMSum_annotated_v5.tsv clinvar_result_04202023_LARGE1.txt LARGE1_conditions
...
# make a header
outFile.write("chr"+'\t'+"hg38_site"+'\t'+"site"+'\t'+"codon"+'\t'+"WT_nt"+'\t'+"Variant_nt"+'\t'+"WT_AA"+'\t'+"Variant_AA"+'\t'+"classification"+'\t'+"fitness_normalized"+'\t'+"Clinvar_clinical_significance"+'\t'+"Clinvar_conditions"+'\n')
...
for i in range(1, len(in_tsv_lines)):
    ...
    chr=tsv_this_line[1]
    hg38_site=tsv_this_line[2]
    site=tsv_this_line[3]
    codon=tsv_this_line[4]
    WT_nt=tsv_this_line[5]
    Variant_nt=tsv_this_line[6]
    WT_AA=tsv_this_line[7]
    Variant_AA=tsv_this_line[8]
    classification=tsv_this_line[12]
    functional_score=tsv_this_line[36]
    Clinvar_clinical_significance=tsv_this_line[37]
    confidence=tsv_this_line[13]

    if confidence=="HIGH" and ((Clinvar_clinical_significance=="Pathogenic") | (Clinvar_clinical_significance=="Pathogenic/likely pathogenic") | (Clinvar_clinical_significance=="Likely pathogenic")):
