#Rscript --vanilla 1_onset_FR2_revise.r FKRPregstry_report_and_SMuRF_diumsum report_and_SMuRF_high_confidence_dimsum
...
df$Cohorts=ifelse(1==1,"GRASP-LGMD", "I am feeling lucky")
df2$Cohorts=ifelse(1==1,"Other Cohorts", "I am feeling lucky")
...
r = ggplot(df,aes(x=as.numeric(Onset), y=log2(2^as.numeric(fs1)+2^as.numeric(fs2)),color=Cohorts))+ geom_point(size=6) + theme_bw()
...
r = r + theme(axis.text.y=element_text(size=18,colour="black",family="Atlas Grotesk Web Bold"),axis.title.y=element_text(size=18,colour="black",family="Atlas Grotesk Web Bold",vjust=1.5))
r = r + theme(axis.text.x=element_text(size=18,colour="black",family="Atlas Grotesk Web Bold"),axis.title.x =element_text(size=18,colour="black",family="Atlas Grotesk Web Bold",vjust=1))
...
r = r + theme(legend.position=c(0.85, 0.3),legend.title=element_text(colour="black",size=10))
r = r + theme(legend.background=element_rect(linetype="solid",colour="black",fill="NA"))


ggsave(filename=paste(basename(args[1]),".onset2_revise.png",sep = ""),dpi=300, heigh=3)
