#Rscript --vanilla 1_bar_main_revise.r report_and_SMuRF_high_confidence_dimsum
...
r = ggplot(df, aes(x=factor(Conditions,order), y=log2(2^as.numeric(fs1)+2^as.numeric(fs2)),color=Sex)) + theme_bw()



r = r + geom_violin(lwd=1) + geom_signif(comparisons = list(c("Intermediate", "Mild")),y_position=0.6,tip_length = c(0.05, 0.05),size=1,textsize=7,color="black")+ geom_signif(comparisons = list(c("Severe", "Mild")),y_position=0.2,tip_length = c(0.05, 0.05),size=1,textsize=7,color="black")+ geom_signif(comparisons = list(c("Severe", "Intermediate")),y_position=-0.2,tip_length = c(0.05, 0.05),size=1,textsize=7,color="black")
r = r + geom_dotplot(fill="white",binaxis='y', stackdir='center',position=position_dodge(0.9),dotsize=0.5)
r = r + ylab("Log2(2^FS1+2^FS2)") + theme(axis.text.x=element_text(size=12))
r = r + geom_hline(yintercept=1, linetype="dashed", color="black", size=0.5) + scale_x_discrete(name="")
...
give.n <- function(x){
  return(c(y = -2.5, label =length(x), size=10))
}
...
r = r + theme(legend.text=element_text(colour="black",size=16,family="Atlas Grotesk Web Bold"))


r = r + theme(legend.position=c(0.1, 0.88),legend.title=element_text(colour="black",size=18,family="Atlas Grotesk Web Bold"))
...
ggsave(filename=paste(basename(args[1]),".bar_main_revise.png",sep = ""),dpi=300)
