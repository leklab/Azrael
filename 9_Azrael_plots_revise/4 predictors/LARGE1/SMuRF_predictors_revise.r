...
#1#
#1#s = ggplot(scores_missense, aes(x=EVE, y=fitness_normalized)) + theme_bw()
...
#5#
s = ggplot(scores_missense, aes(x=as.numeric(alphamissense), y=fitness_normalized)) + theme_bw()


#1#
#1#s = s + ylab("SMuRF score")+xlab("EVE score")
...
#5#
s = s + ylab("SMuRF score")+xlab("AlphaMissense")
...
#1#
#1#ggsave(filename=paste(basename(args[1]),".EVE_high_confi.png",sep = ""),dpi=300, height=10)
...
#5#
ggsave(filename=paste(basename(args[1]),".AlphaMissense_high_confi.png",sep = ""),dpi=300, height=10)
