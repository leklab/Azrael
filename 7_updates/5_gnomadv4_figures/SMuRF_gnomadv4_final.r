#Rscript --vanilla SMuRF_gnomadv4_final.r gnomad_fkrp-2_annotated_v4.tsv
#Rscript --vanilla SMuRF_gnomadv4_final.r Supplementary\ Table\ 4_updated_v6.tsv
...
scatterplot <- ggplot(df, aes(x = gnomad_v4_genome_exome_af, y = log2(functional_score), color = classification)) +
  ...
  theme(plot.title = element_text(size = 14, hjust = 0, vjust = 1, family = "Helvetica", face = "bold"),plot.margin = margin(5, 5, 5, 5)) + theme_bw()
