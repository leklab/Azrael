#Rscript --vanilla predict_heat.r Supplementary\ Table\ 4_updated_v6.tsv
#Rscript --vanilla predict_heat.r Supplementary\ Table\ 5_updated_v7.tsv
...
columns_to_keep <- c("functional_score", "am_pathogenicity", "PrimateAI_3D", "EVE", "revel_score", "MutScore","ESM1b_score","cadd","Maverick_RecScore", "meta_svm_score", "mvp_rankscore")
...
heatmap_plot <- ggplot(data = melted_data, aes(Var1, Var2, fill = value)) +
  ...
  labs(x =element_blank(), y =element_blank(), fill = "Abs. Spearman's Rho", title="FKRP") +theme_bw()+
...
heatmap_plot = heatmap_plot + theme(legend.background=element_rect(linetype="solid",colour="black"))


ggsave(filename=paste(basename(args[1]),".heatmap.png",sep = ""),dpi=300)
