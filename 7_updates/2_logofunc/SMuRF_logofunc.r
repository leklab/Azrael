#Rscript --vanilla SMuRF_logofunc.r Supplementary\ Table\ 5_updated_v5.tsv
...
# Main plot
scatterplot <- ggplot(df, aes(x = as.numeric(LoGoFunc_GOF), y = log2(functional_score), color=logofunc_pred,label=variantinfo)) +
  geom_point(size = 2) +geom_text_repel(aes(label=ifelse(as.numeric(df$LoGoFunc_GOF)>=0.3 & log2(df$functional_score)>=1.5 & df$logofunc_pred=="GOF",as.character(variantinfo),'')),size=3, face="bold")+
