...
r = ggplot(df,aes(x=as.numeric(CK_IU_L_ave), y=log2(as.numeric(fs1)+as.numeric(fs2)),color=Sex))+ geom_point(size=6)+ geom_smooth(method=lm,se=FALSE, size=1,linetype="dashed") + theme_bw()
...
dfm=df[df$Sex == "M",]
dff=df[df$Sex == "F",]
cor.test(log2(as.numeric(df$fs1)+as.numeric(df$fs2)), as.numeric(df$CK_IU_L_ave),method="spearman")
cor.test(log2(as.numeric(dfm$fs1)+as.numeric(dfm$fs2)), as.numeric(dfm$CK_IU_L_ave),method="spearman")
cor.test(log2(as.numeric(dff$fs1)+as.numeric(dff$fs2)), as.numeric(dff$CK_IU_L_ave),method="spearman")
